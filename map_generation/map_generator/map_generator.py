...
class MapIndividual():
    ...
    def set_all_blank(self):
        for i in range(1, self.msize-1):
            for j in range(1, self.msize-1):
                self.matrix[i][j] = tiles_dict["food"]

    def get_fitness(self):
        score = 100.0
        score += 500.0 * int(self.is_playable())
...
class GeneticAlgorithm():
    ...
    def evolve(self):
        for iteration in range(self.iterations):
            ...
            for i in range(children_start_index, self.psize, 2):
                ...
                self.population[i], self.population[i+1] = self.crossover(parent1, parent2)
